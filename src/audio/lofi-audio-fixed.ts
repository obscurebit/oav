/**
 * Fixed Lo-Fi Audio System - Simple wrapper for the fixed lo-fi music system
 */

import { LoFiMusicSystemFixed } from './lofi-music-fixed';

export class LoFiAudioFixed {
  private _musicSystem: LoFiMusicSystemFixed;
  ...
  constructor() {
    this._musicSystem = new LoFiMusicSystemFixed();
  ...
  init(): void {
    ...
    console.log('[LoFiAudioFixed] Initialized lo-fi audio system');
  }
  ...
  private _connectMusicSystemToAnalyser(): void {
    ...
    if (musicSystem._masterGain) {
      ...
      console.log('[LoFiAudioFixed] Connected music system through analyser');
    } else {
      console.warn('[LoFiAudioFixed] Could not find music system master gain');
    }
  }
  ...
  start(): void {
    ...
    console.log('[LoFiAudioFixed] Started lo-fi music');
  }
  ...
  stop(): void {
    ...
    console.log('[LoFiAudioFixed] Stopped lo-fi music');
  }
  ...
  triggerKick(volume: number = 0.5): void {
    console.log('[LoFiAudioFixed] Kick trigger (handled by music system)');
  }
  
  triggerSnare(volume: number = 0.5): void {
    console.log('[LoFiAudioFixed] Snare trigger (handled by music system)');
  }
  
  triggerHihat(volume: number = 0.5): void {
    console.log('[LoFiAudioFixed] Hi-hat trigger (handled by music system)');
  }
  
  triggerBass(volume: number = 0.5): void {
    console.log('[LoFiAudioFixed] Bass trigger (handled by music system)');
  }
  
  /** Trigger drum pattern (for compatibility). */
  triggerDrumPattern(pattern: string, volume: number = 0.5): void {
    console.log(`[LoFiAudioFixed] Drum pattern trigger: ${pattern} (handled by music system)`);
  }
  
  /** Trigger SFX (for compatibility). */
  triggerSFX(sfx: any): void {
    console.log(`[LoFiAudioFixed] SFX trigger: ${sfx.type} (lo-fi focuses on music)`);
  }
  
  /** Set parameters (for compatibility). */
  setParams(params: any): void {
    if (params.tempo) {
      console.log(`[LoFiAudioFixed] Tempo set to ${params.tempo} (handled by theme)`);
    }
  }
